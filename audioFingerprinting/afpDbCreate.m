...
%	Roger Jang, 20130731, 20160205, 20160922
...
% ====== Collect audio files
auExt={'mp3', 'wav'};	% Extension of audio files to be collected
audioData=[];
for i=1:length(auExt)
	audioData=[audioData; recursiveFileList(audioDir, auExt{i})];
end
...
for i=1:length(audioData)
	myTic=tic;
	[parentDir, audioData(i).mainName]=fileparts(audioData(i).path);
	...
	lmList=afpFeaExtract(y, fs, afpOpt);		% Row of lmList = [t1 f1 f2 t2-t1]
	...
	hashKey=hashList(:,1)+1;		% Convert to MATLAB indexing
